...
// DBTITLE 1,create the csv file

dbutils.fs.put("/mydata/emp.csv","""
...
val df = spark.read.option("header",true).csv("/mydata/emp.csv")
display(df)
...
// MAGIC %sql
// MAGIC
// MAGIC create database dbs
// MAGIC

// COMMAND ----------


df.write.mode("append").format("delta").saveAsTable("dbs.emp")
...
// MAGIC %sql
// MAGIC
// MAGIC select * from dbs.emp where DEPTNO=10
// MAGIC
...
// MAGIC %sql 
// MAGIC
// MAGIC describe history dbs.emp

// COMMAND ----------

// MAGIC %sql
// MAGIC
// MAGIC select * from dbs.emp version as of 0
// MAGIC
...
// MAGIC %sql
// MAGIC
// MAGIC restore dbs.emp version as of 0
// MAGIC
// MAGIC
